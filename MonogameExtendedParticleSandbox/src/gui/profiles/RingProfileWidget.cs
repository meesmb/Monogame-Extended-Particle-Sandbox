...
{
    public class RingProfileWidget : ProfileWidget
    {
        ...
        {
            var profile = Profile.Ring(radiusValue, radiation);
            ...
            var combo = GUI.createComboBox(grid, 1, 1, GUI.convertDictionaryToList(circleRadiations), (v, ComboBox) =>
            {
                ...
                particleEmitter.Profile = Profile.Ring(radiusValue, radiation);
            }, 0);


        }

        public RingProfileWidget()
            : base(null, 0, null)
        {
        ...
        {
            return new RingProfileWidget(parent, row, particleEmitter);
        }
    }
}
